 ...
 * @file disp_spi.c
 *
 * DMA SPI transport for the ILI9341 — ported from the vendor's
 * lvgl_esp32_drivers component and upgraded for ESP-IDF v6 / LVGL 9.3.
 *
 * A pool of reusable spi_transaction_ext_t structures (in DMA-capable
 * memory) is kept in an xQueue.  Each DMA transfer takes a transaction out
 * of the pool, queues it, and the post-transfer callback (spi_ready)
 * returns it to the pool.  Polling/synchronous sends first drain all pending
 * DMA transactions.
 */
...
/* NOTE: no IRAM_ATTR here. GCC 15 (-Werror=attributes) rejects the IRAM
 * section attribute on this static callback because it clones it into a
 * second section (.iram1.1 vs .iram1.0). It is a no-op in this project
 * (chained_post_cb is NULL), so running from flash is fine. */
static void spi_ready(spi_transaction_t *trans)
{
    /* LVGL v9: flush-ready is signalled from the app's flush callback after
     * disp_wait_for_pending_transactions(), so nothing to do here besides
     * chaining a user post-callback (unused by this project). */
    (void) trans;
    if (chained_post_cb) {
